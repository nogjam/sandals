...
# NOTE: datetime is a subclass of date
type PodType = type[bool | int | float | str | date]
type PodData = bool | int | float | str | date
...
SQLITE_POD_TYPE_MAP: t.Final[dict[type, PodSqlite]] = {
    ...
    date: PodSqlite(type_=date, sql="TEXT"),
    datetime: PodSqlite(type_=datetime, sql="TEXT"),
}
...
class Field:
    def __init__(
        ...
        name: str,
        py_type: type,
        unmarshal: t.Callable[[t.Any], t.Any],
        sql_type: str,
        kind: Kind,
    ) -> None:
        self.name = name
...
class DataClass:
            ...
            if f.kind is Kind.DC:
                ...
                try:
                    if isinstance(v, list):
                        cast[f.name] = list(map(f.unmarshal, v))
                    else:
                        cast[f.name] = f.unmarshal(v)
                except TypeError as e:
                    raise TypeError(
                        f"Field {f.name!r} could not be cast to {f.py_type.__name__} from {type(v).__name__}"
                    ) from e
    ...
    def marshal_values_pod(self) -> tuple[PodData, ...]:
        def marshal_single_field(f: Field) -> t.Any:
            if isinstance(p := getattr(self, f.name), PodWrapper):
                return getattr(p, MARSHAL_METHOD_NAME)()
            elif isinstance(p, date):
                return str(p)
            else:
                return p

        return tuple(marshal_single_field(f) for f in self.fields_pod)
...
def select_all_records[T: DataClass](
    ...
    for row in records:
        ...
        obj_kwargs: dict[str, PodData | DataClass | StructuredData | CompoundData] = {
            f_p.name: f_p.unmarshal(v) for f_p, v in zip(data_cls.fields_pod, row)
        }
